...
NTSTATUS NTAPI hkNtOpenThread(PHANDLE ThreadHandle, ACCESS_MASK AccessMask, POBJECT_ATTRIBUTES ObjectAttributes, PCLIENT_ID pClientId)
{
	if(pClientId->UniqueThread == Breakpoint->GetMainThreadId(GetCurrentProcessId()) && !(AccessMask & THREAD_SUSPEND_RESUME))
	{
		NTSTATUS ret = oNtOpenThread(ThreadHandle, AccessMask, ObjectAttributes, pClientId);
		*ThreadHandle = fakeHandle;
		return ret;
	}

	return oNtOpenThread(ThreadHandle, AccessMask, ObjectAttributes, pClientId);
}

CBreakpoint::CBreakpoint()
{
	oNtOpenThread = (tNtOpenThread)DetourFunction((PBYTE)GetProcAddress(GetModuleHandleA("ntdll.dll"), "NtOpenThread"), (PBYTE)hkNtOpenThread);
	fakeHandle = CreateThread(NULL, NULL, FakeThread, NULL, NULL, NULL);

	AddVectoredExceptionHandler(1, UnhandledExceptionFilterGateway);
}
